...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not all([mobile, password]):
            return self.write({"errno":RET.PARAMERR, "errmsg":"参数错误"})
        res = self.db.get("select up_user_id,up_name,up_passwd,up_user_id,up_avatar,up_real_name,up_id_card from ih_user_profile where up_mobile=%(mobile)s", mobile=mobile)
        password = hashlib.sha256(config.passwd_hash_key + password).hexdigest()
...
class LogoutHandler(BaseHandler):
    ...
    def get(self):
        try:
            ...
            logging.error(e)
            return self.write({"errno":RET.DATAERR, "errmsg":"登出失败"})

        return self.write({"errno":RET.OK, "errmsg":"OK"})
        # self.redis.deleter("user_id")
